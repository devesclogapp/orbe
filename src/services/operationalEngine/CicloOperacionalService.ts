...
export class CicloOperacionalService {
   ...
   */
  static async validarFinanceiro(cicloId: string, usuarioId: string, observacao: string = 'Valida��o financeira conclu�da'): Promise<CicloOperacional> {
    const { data: ciclo, error: errFetch } = await supabase
      .from('ciclos_operacionais')
    ...
    if (ciclo.status !== 'fechado') {
      throw new Error("O ciclo precisa estar fechado operacionalmente antes da valida��o Financeira.");
    }

    if (ciclo.status_rh !== 'validado_rh') {
       throw new Error("O ciclo precisa estar validado pelo RH antes da valida��o Financeira.");
    }

    if ((ciclo.total_inconsistencias || 0) > 0) {
      throw new Error("N�o � poss�vel validar no Financeiro: existem inconsist�ncias no ciclo.");
    }
