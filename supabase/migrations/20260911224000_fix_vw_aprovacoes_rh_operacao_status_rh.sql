-- ==============================================================================
-- Migration: Fix public.vw_aprovacoes_rh for OPERAÇÃO records
-- Objective: Map raw_status from op.status_rh instead of op.status for OPERAÇÃO.
-- Context: Operações por Volume track RH approval state in operacoes_producao.status_rh.
--          Using op.status (which holds operational/financial status like AGUARDANDO_FATURAMENTO)
--          caused approved operations to remain in 'Em análise' instead of moving to 'Aprovado'.
-- Date: 2026-09-11
-- ==============================================================================
...
WITH combined AS (
    ...
    SELECT 
        ...
        COALESCE(op.status_rh, 'PENDENTE_RH') AS raw_status,
