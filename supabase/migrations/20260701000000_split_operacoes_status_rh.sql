-- Migration: Split RH and Faturamento pipelines for operacoes_producao
-- Objective: Add status_rh and update operational status pipeline without breaking existing data

-- 1. Add status_rh column
ALTER TABLE public.operacoes_producao ADD COLUMN IF NOT EXISTS status_rh VARCHAR(50) DEFAULT 'PENDENTE_RH';
ALTER TABLE public.operacoes_producao DROP CONSTRAINT IF EXISTS operacoes_producao_status_rh_check;
ALTER TABLE public.operacoes_producao ADD CONSTRAINT operacoes_producao_status_rh_check 
  CHECK (status_rh IN ('PENDENTE_RH', 'EM_ANALISE_RH', 'VALIDADO_RH', 'DEVOLVIDO_RH'));

-- 2. Backfill status_rh based on current status
-- Assume all closed / processed ops imply RH was validated
UPDATE public.operacoes_producao 
SET status_rh = 'VALIDADO_RH'
WHERE status IN ('validado_rh', 'fechado', 'processado', 'aprovado_financeiro', 'concluido', 'aguardando_faturamento');

UPDATE public.operacoes_producao 
SET status_rh = 'DEVOLVIDO_RH'
WHERE status IN ('recusado', 'devolvido', 'com_alerta', 'bloqueado');

UPDATE public.operacoes_producao 
SET status_rh = 'PENDENTE_RH'
WHERE status_rh NOT IN ('VALIDADO_RH', 'DEVOLVIDO_RH', 'EM_ANALISE_RH');

-- 3. Update main operational status
-- Drop old constraint
ALTER TABLE public.operacoes_producao DROP CONSTRAINT IF EXISTS operacoes_producao_status_check;

-- Backfill old status to new operational flow
UPDATE public.operacoes_producao SET status = 'CONCLUIDO' WHERE status IN ('concluido', 'fechado', 'processado', 'pago');
UPDATE public.operacoes_producao SET status = 'AGUARDANDO_FATURAMENTO' WHERE status IN ('aprovado_financeiro', 'aguardando_pagamento', 'validado_rh');
UPDATE public.operacoes_producao SET status = 'EM_RESTRICAO' WHERE status IN ('com_alerta', 'bloqueado', 'recusado', 'devolvido', 'inconsistente');
UPDATE public.operacoes_producao SET status = 'EM_VALIDACAO' WHERE status IN ('aguardando_validacao', 'em_analise', 'pendente');
UPDATE public.operacoes_producao SET status = 'RECEBIDO' WHERE status NOT IN ('CONCLUIDO', 'AGUARDANDO_FATURAMENTO', 'EM_RESTRICAO', 'EM_VALIDACAO', 'RECEBIDO');
...
-- 4. Recreate vw_aprovacoes_rh to use status_rh
DROP VIEW IF EXISTS public.vw_aprovacoes_rh;
...
SELECT 
    c.*,
    CASE
        WHEN upper(c.raw_status) IN ('EM_ABERTO', 'PENDENTE', 'AGUARDANDO_VALIDACAO_RH', 'EM_ANALISE', 'DETALHADO', 'REGISTRADO', 'PENDENTE_RH', 'EM_ANALISE_RH') THEN 'Em análise'
        ...
        WHEN upper(c.raw_status) IN ('DEVOLVIDO', 'CANCELADO', 'CANCELADO_RH', 'RETORNADO', 'RECUSADO', 'REPROVADO', 'DEVOLVIDO_RH') THEN 'Devolvido'
        ELSE 'Em análise'
    END AS situacao
FROM combined c;
